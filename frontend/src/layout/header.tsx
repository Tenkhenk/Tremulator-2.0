...
const Header = () => {
  ...
  const currentIndex:number = findIndex(currentCollection?.images, (i:ImageType) => i.id.toString() === currentImageID);
  const previousImageID:number|null = currentCollection && currentIndex > 0 ? currentCollection.images[currentIndex - 1].id : null
  const nextImageID:number|null = currentCollection && currentIndex < currentCollection.images.length - 1 ? currentCollection.images[currentIndex + 1].id : null
  ...
  return (
    <header>
      <nav className="navbar container-fluid">
          ...
          {currentCollection && <div className=".navbar-text">
             <h4 className="mx-auto"><Link to={`/collections/${currentCollection.id}`}>{currentCollection.name}</Link> <Link to={`/collections/${currentCollection.id}/edit`} title="edit"><i className="fas fa-edit"></i></Link></h4>
